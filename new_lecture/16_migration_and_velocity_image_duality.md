...
- **[LO-16.1]** Explain why a zero-offset seismic section mispositions dipping reflectors, and derive the geometric corrections $\Delta x = d\sin\theta$ and $\tau = t\cos\theta$.
- **[LO-16.2]** State the exploding-reflector analogy, explain why it requires the propagation velocity to be halved, and identify two situations in which it breaks down.
- **[LO-16.3]** Describe the Kirchhoff adjoint pair — that forward modeling spreads a scatterer into a data-space hyperbola, while migration sums data along the same hyperbola back into model space — and use this to explain why modeling and migration are transposes of each other.
- **[LO-16.4]** Argue from a migrated image whether the migration velocity was correct, too slow, or too fast, using the geometry of residual curvature.
- **[LO-16.5]** Diagram how refraction (first-arrival tomography) and reflection (NMO/migration) imaging share a single Earth model $v(x,z)$, and articulate how each method constrains a different part of it.
- **[LO-16.6]** Identify one task in the active-source processing chain where deep learning provides a useful surrogate, and state explicitly what physical knowledge was required to produce the surrogate's training data.
...
| **Prior lecture** | Lecture 14 — Seismic Reflections I (reflection coefficients, NMO, CMP stacking) |
| **Next lecture** | Lecture 17 — Whole Earth Structure I |
| **Lab connection** | Lab 4 (4/24): Design → Simulate → Image — students build a synthetic dataset from their own survey design and migrate with correct and deliberately incorrect velocities |
...
Students should be comfortable with: ray geometry and Snell's law (Lectures 6–7); refraction head waves and intercept-time analysis (Lectures 9–11); reflection coefficients, normal moveout, and CMP stacking (Lectures 12–14); the hyperbolic travel-time curve $t(x) = \sqrt{t_0^2 + x^2/V_{\rm rms}^2}$.
...
A single shot gather in {numref}`fig-integrated-shot-gather` contains five distinct event families — a direct wave, two reflection hyperbolas, and two head waves. Prior lectures treated these separately: refraction in Lectures 9–11, reflection in Lectures 12–14. This lecture unifies them. The claim is simple: every method of active-source seismology is a strategy for turning travel-time data into an Earth model, and all of them share the same underlying problem — the *image* depends on the *velocity*, and the velocity is inferred from the same data.
...
which is the zero-offset analogue of Snell's parameter $p = \sin\theta/v$ familiar from Lecture 7, with a factor of two for the two-way path. Substituting {eq}`eq-tuchel` into {eq}`eq-deltax` and {eq}`eq-tau` yields the hand-migration formulas
...
Migration requires a velocity model $v(x,z)$. Where does that velocity come from? In the shallow section it comes from refraction first-arrival tomography (Lecture 11): head-wave travel times, inverted with {cite:t}`ZeltBarton1998` -style tomography or its modern equivalents, constrain $v(z)$ down to the depth of the deepest refractor. In the deeper section it comes from reflection moveout analysis (Lecture 14): hyperbolic semblance picking yields $V_{\rm rms}(t_0)$, which the Dix equation converts to interval velocities.
...
- **Lecture 7 — Snell's law.** Equation {eq}`eq-tuchel` is the zero-offset version of Snell's parameter; the factor of two traces to the round-trip path.
- **Lectures 9–11 — Seismic refraction.** First-arrival times and the intercept-time equation provide the shallow velocity model that seeds migration.
- **Lectures 12–14 — Seismic reflection.** NMO, CMP stacking, and Dix inversion provide the deeper part of the same velocity model.
- **Lecture 17 — Whole-Earth structure.** The same physical framework — travel times through a layered medium — produces the global 1-D Earth model (PREM) by stacking seismograms from earthquakes instead of shots.
- **Lecture 21 — Seismic tomography.** The local refraction/reflection velocity model is the small-scale analogue of the global tomographic models; both solve $\mathbf{d} = F(\mathbf{m})$ for $\mathbf{m}$.
- **Lab 4 (Friday, 4/24) — Design → Simulate → Image.** Students take the survey design from the companion discussion session, generate a synthetic shot gather, migrate it with three velocities, and report which velocity is correct and how they could tell from the image alone.
